...
const ProductCardItem = ({ product }: { product: ProductItemType }) => {
    return (
        <article className="relative shadow-sm transition-all duration-200 ease-linear rounded-sm
        flex flex-col items-center justify-start gap-y-1 pb-2 max-sm:pb-1.5
        bg-muted mx-2 h-full group">
            <Link className="w-full h-full relative" href={`/products/${product.id}`}>
                <CategoryBadge category={product.category} />
                {/* Rating Icon Based on Rating value 3 types */}
                <RatingSingleIcon rating={product.rating} />
                <Image
                    className="mx-auto relative group-hover:scale-105 transition-transform duration-200 ease-linear"
